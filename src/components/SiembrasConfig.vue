          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="w-full sm:w-auto z-10">
              <v-btn
                ...
                :aria-label="'Crear nueva siembra o proyecto agrícola'"
                tabindex="0"
              >
                Nueva Siembra
              </v-btn>
        ...
        <v-row>
          ...
          <v-col v-for="siembra in siembras" :key="siembra.id" cols="12" sm="6" md="4" lg="3">
            <v-card
              ...
              elevation="2"
              :aria-label="`Abrir siembra ${siembra.nombre} ${siembra.tipo}, estado ${siembra.estado}`"
              role="button"
              tabindex="0"
            >
              <!-- Unified compliance header for siembras -->
                ...
                <div class="d-flex align-center justify-space-between">
                  <div class="d-flex align-center">
                    <v-icon
                      ...
                      size="large"
                      class="mr-3"
                      :aria-label="`Estado de siembra: ${siembra.estado}`"
                    >
                      {{ getSiembraStateIcon(siembra.estado) }}
                    </v-icon>
                    <div>
                      <div class="compliance-title">{{ siembra.nombre }} {{ siembra.tipo }}</div>
                      <div class="compliance-subtitle">Iniciada: {{ formatDate(siembra.fecha_inicio) }}</div>
                    </div>
                  </div>

                  <!-- Status and zone chips -->
                  <div class="d-flex align-center ga-2 flex-wrap">
                    ...
                    <v-chip
                      v-if="getZoneNames(siembra).length > 0 && !getZoneNames(siembra).includes('Sin zonas asignadas')"
                      color="info"
                      size="small"
                      variant="outlined"
                      class="agricultural-chip"
                    >
                      ...
                      {{ getZoneNames(siembra).length }} zona(s)
                    </v-chip>
                  </div>
                </div>
              </div>
              ...
              <v-card-text class="agricultural-content pa-0">
                <v-img
                  ...
                  class="siembra-image"
                >
                  <!-- Zone details overlay (if zones exist) -->
                  <div
                    v-if="getZoneNames(siembra).length > 0 && !getZoneNames(siembra).includes('Sin zonas asignadas')"
                    class="info-overlay"
                  >
                    <div class="d-flex flex-wrap ga-1">
                      ...
                      <v-chip
                        ...
                        class="agricultural-chip agricultural-chip--zone-overlay"
                      >
                        +{{ getZoneNames(siembra).length - 3 }} más
                      </v-chip>
                    </div>
                  </div>
                </v-img>
                ...
                <div class="pa-3">
                  <v-btn
                    ...
                    :aria-label="`Abrir detalles de siembra ${siembra.nombre}`"
                  >
        ...
        <v-form @submit.prevent="crearSiembra">
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              :aria-label="'Cancelar creación de siembra'"
              tabindex="0"
            >
              Cancelar
            </v-btn>
            <v-btn
              ...
              :aria-label="'Confirmar creación de nueva siembra'"
              tabindex="0"
            >
              Crear Siembra
            </v-btn>
...
// New computed properties for compliance header system
...
/* Unified Compliance Header System */
.compliance-header {
...
/* Siembra-specific header variations */
